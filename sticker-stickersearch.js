...
async function urlToSticker(url, packname, author) {
  ...
  if (!res.ok) throw new Error('لم أتمكن من تحميل الصورة')
  const img = await res.buffer()
  const type = await fileTypeFromBuffer(img)
  if (!type) throw new Error('نوع الملف غير صالح')
...
let handler = async (m, { conn, text, command }) => {
  ...
  try {
    ...
    if (!jsonBusqueda.status || !Array.isArray(jsonBusqueda.data) || jsonBusqueda.data.length === 0) {
      return m.reply('✿ لم أجد ملصقات بهذا الاسم ✦')
    }
    ...
    if (!jsonDetalle.status || !jsonDetalle.data || !Array.isArray(jsonDetalle.data.stickers) || jsonDetalle.data.stickers.length === 0) {
      return m.reply('✦ حدث خطأ أثناء جلب الملصقات ✐')
    }

    let nombrePack = jsonDetalle.data.name || 'بدون اسم'
    let autorPack = jsonDetalle.data.author?.name || 'غير معروف'

    await m.reply(`❐ سيتم إرسال 5 ملصقات من:\n✧ *${nombrePack}*\nبواسطة ✿ *${autorPack}*`)
    ...
    console.error(err)
    m.reply('❀ حدث خطأ أثناء معالجة الملصقات ✧')
  }
}

handler.help = ['ملصق_بحث *<النص>*']
handler.tags = ['sticker']
handler.command = ['ملصق_بحث']
