...
let handler = async (m, { conn, text, args }) => {
  if (!text) return m.reply(`🤍 أدخل النص. مثال: .بنترست bmw`);
  ...
  try {
    if (text.includes("https://")) {
      ...
      await conn.sendMessage(
        m.chat,
        { [isVideo ? "فيديو" : "صوره"]: { url: i.download }, caption: i.title, ...rcanal },
        { quoted: fkontak }
      );
      m.react("☑️");
    } else {
    ...
    conn.sendMessage(m.chat, { text: 'خطأ في الحصول على الصور من Pinterest:\n\n' + e, ...rcanal }, { quoted: m });
  }
};

handler.help = ['بنتريست'];
handler.command = ['بنتريست', 'pin'];
